...
public class LocationIndexController {
	...
	public ResponseEntity<Void> postLocationsToSearchIndex(
			...
			@Valid @RequestBody LocationQueryResult body) {
	...
	public ResponseEntity<LocationQueryResult> searchSearchKeywordGet(
		...
		LocationQueryResult locationQueryResult = LocationQueryResult.builder().locations(page.getContent())
				.totalElements(page.getTotalElements()).build();
		return new ResponseEntity<>(locationQueryResult, HttpStatus.OK);
	}
